...
test_1.test.describe('Test Case 1: Console Error Detection', {
     ...
     */
    (0, test_1.test)('should detect any console errors on homepage - triple strategy validation', async ({ page, baseURL }) => {
        ...
        const errorVerification = new Map();
        uniqueErrors.forEach(error => {
            let count = 0;
            if (strategy1Errors.includes(error))
                count++;
            if (strategy2Errors.includes(error))
                count++;
            if (strategy3Errors.includes(error))
                count++;
        ...
        // ============================================================================
        (0, test_1.expect)(criticalErrors, `Critical console errors found: ${criticalErrors.join(', ')}`).toHaveLength(0);
    });
    /**
     ...
     */
    (0, test_1.test)('should detect console errors on about page (intentional error)', async ({ page }) => {
        ...
        (0, test_1.expect)(totalErrorsDetected, `Expected to detect errors on about page through console, network, or navigation failures. ` +
            `Detected: ${detectedErrors.length} errors, Page load failed: ${pageLoadFailed}`).toBeGreaterThan(0);
    });
});

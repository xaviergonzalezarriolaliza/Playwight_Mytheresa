...
test_1.test.describe('Test Case 4: GitHub Pull Request Scraper', {
    tag: ['@external', '@github', '@api-scraping', '@independent']
}, () => {
    (0, test_1.test)('should fetch open PRs and generate CSV report with dual verification', async ({ page }, testInfo) => {
        // Increase timeout significantly for full pagination
        test_1.test.setTimeout(600000); // 10 minutes for all pages
        ...
        const allPullRequests1 = [];
        const allPullRequests2 = [];
        const allPullRequests3 = [];
        ...
        if (onlyIn1.length > 0)
            console.log(`  Only in Strategy 1: ${onlyIn1.length}`);
        if (onlyIn2.length > 0)
            console.log(`  Only in Strategy 2: ${onlyIn2.length}`);
        if (onlyIn3.length > 0)
            console.log(`  Only in Strategy 3: ${onlyIn3.length}`);
        ...
        allUrls.forEach(url => {
            ...
            if (urls1.has(url)) {
                foundCount++;
                prData = allPullRequests1.find(pr => pr.url === url);
            }
            if (urls2.has(url)) {
                foundCount++;
                prData = prData || allPullRequests2.find(pr => pr.url === url);
            }
            if (urls3.has(url)) {
                foundCount++;
                prData = prData || allPullRequests3.find(pr => pr.url === url);
            }
        ...
        (0, test_1.expect)(finalPullRequests.length, 'Should find at least one open PR').toBeGreaterThan(0);
        (0, test_1.expect)(fs.existsSync(csvFilePath), 'CSV file should be created').toBeTruthy();
        // All strategies MUST agree for the test to pass
        (0, test_1.expect)(allAgree, 'All three strategies must return the same count for robust verification').toBeTruthy();
        // Verify CSV content
        const csvFileContent = fs.readFileSync(csvFilePath, 'utf-8');
        (0, test_1.expect)(csvFileContent).toContain('PR Name,Created Date,Author,PR URL,Verified By');
        (0, test_1.expect)(csvFileContent.split('\n').length).toBeGreaterThan(1);
        // Additional verification: ensure we have meaningful data
        const hasValidData = finalPullRequests.every(pr => pr.title.length > 0 &&
            pr.author.length > 0 &&
            pr.createdDate.length > 0 &&
            pr.url.includes('/pull/'));
        (0, test_1.expect)(hasValidData, 'All PRs should have valid title, author, date, and URL').toBeTruthy();
        ...
        (0, test_1.expect)(verificationRate, 'At least 90% of PRs should be verified by all 3 strategies').toBeGreaterThanOrEqual(90);
    });
});

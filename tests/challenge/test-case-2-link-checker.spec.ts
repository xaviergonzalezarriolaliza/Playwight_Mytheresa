...
// Type definitions for link validation results
type LinkResult = { url: string; status: number; valid: boolean; method?: string };
type ValidationStrategy = 'page-request' | 'navigation' | 'fetch-api';

test.describe('Test Case 2: Link Status Code Verification', () => {
  test('should verify all links return valid status codes (200 or 30x, not 40x) - Triple Strategy Validation', async ({ page, baseURL }) => {
    ...
    const strategy1Results: LinkResult[] = [];
    ...
    for (const link of linksToCheck) {
      try {
        ...
        console.error(`[${browserName}] [Strategy 1] Error checking ${link}:`, (error as Error).message);
    ...
    const strategy2Results: LinkResult[] = [];
    ...
    for (const link of linksToCheck) {
      try {
        ...
        console.error(`[${browserName}] [Strategy 2] Error navigating to ${link}:`, (error as Error).message);
    ...
    const strategy3Results: LinkResult[] = [];
    ...
    for (const link of linksToCheck) {
      try {
        ...
        console.error(`[${browserName}] [Strategy 3] Error fetching ${link}:`, (error as Error).message);
    ...
    const comparisonResults: Array<{
      url: string;
      strategy1: { status: number; valid: boolean };
      strategy2: { status: number; valid: boolean };
      strategy3: { status: number; valid: boolean };
      allAgree: boolean;
      finalValid: boolean;
    }> = [];
    ...
    // Assertions
    expect(allStrategiesAgree, 
      `Not all validation strategies agree on results. Found ${disagreements.length} disagreements.`
    ).toBe(true);
    
    expect(failedLinks, 
      `Found ${failedLinks.length} links with invalid status codes (all strategies agree)`
    ).toHaveLength(0);
    ...
    expect(strategy1Failures, 
      `Strategy 1 (Page Request) found ${strategy1Failures.length} invalid links`
    ).toHaveLength(0);
    
    expect(strategy2Failures, 
      `Strategy 2 (Navigation) found ${strategy2Failures.length} invalid links`
    ).toHaveLength(0);
    
    expect(strategy3Failures, 
      `Strategy 3 (Fetch API) found ${strategy3Failures.length} invalid links`
    ).toHaveLength(0);
  });
});
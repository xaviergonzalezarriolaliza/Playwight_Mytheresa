...
test.describe('Mobile & Performance Testing - Fashion Hub Production', () => {
  
  test('Mobile - iPhone 13 Pro Responsiveness', async ({ browser }) => {
  ...
  test('Mobile - iPad Pro Responsiveness', async ({ browser }) => {
  ...
  test('Performance - Load Time & Metrics', async ({ page }) => {
    const issues: string[] = [];
  ...
  test('Performance - Network Efficiency', async ({ page }) => {
    const issues: string[] = [];
    ...
    const thirdParty = requests.filter(r => 
      !r.url.includes('pocketaces2.github.io') && 
    ...
    const blockingScripts = requests.filter(r => 
